...
# --- FUNCIÓN PARA LOS PLACEHOLDERS (TEXTO EN GRIS) ---
...
class AppFuerzaElectrica(tk.Tk):
    ...
    def _construir_interfaz(self):
        ...
        estilo.configure("TLabelframe.Label", background="#e9ecef")
        ...
        agregar_placeholder(self.ent_q_obj, "1e-6") # <-- APLICADO
        ...
        agregar_placeholder(self.ent_x_obj, "0") # <-- APLICADO
        ...
        agregar_placeholder(self.ent_y_obj, "0") # <-- APLICADO

        # 2. Agregar Cargas Fuente
        ...
        agregar_placeholder(self.ent_qi, "Ej: -5e-6") # <-- APLICADO
        ...
        agregar_placeholder(self.ent_xi, "0.5") # <-- APLICADO
        ...
        agregar_placeholder(self.ent_yi, "0.5") # <-- APLICADO
        ...
        tk.Button(marco_botones_lista, text="➕ Añadir", bg="#0d6efd", fg="white", 
                command=self.agregar_carga, width=10).pack(side="left", padx=5)
        tk.Button(marco_botones_lista, text="🗑️ Eliminar", bg="#dc3545", fg="white", 
                command=self.eliminar_carga, width=10).pack(side="left", padx=5)
        ...
        self.tabla = ttk.Treeview(marco_tabla, columns=("q", "x", "y"), show="headings", height=8, yscrollcommand=scroll.set)
        ...
        self.lbl_vector = tk.Label(marco_res, text="Vector Fuerza: < 0.00, 0.00 > N", font=("Courier", 14, "bold"), bg="#e9ecef", fg="#212529")
        ...
        if MATPLOTLIB_DISPONIBLE:
            ...
            tk.Label(self.marco_grafica, text="Matplotlib no instalado.").pack()

    def agregar_carga(self):
        try:
            # Si el usuario no escribió nada (quedó el placeholder), esto lanzará ValueError
            ...
            self.ent_qi.delete(0, tk.END)
            self.ent_xi.delete(0, tk.END)
            self.ent_yi.delete(0, tk.END)
            agregar_placeholder(self.ent_qi, "Ej: -5e-6")
            agregar_placeholder(self.ent_xi, "0.5")
            agregar_placeholder(self.ent_yi, "0.5")

        except ValueError:
            messagebox.showerror("Error", "Ingresa valores numéricos válidos. No dejes los ejemplos grises.")
    ...
    def calcular_fuerza(self):
        try:
            ...
            if not self.cargas_fuente:
                messagebox.showerror("Error", "¡Necesitas al menos una carga fuente!")
                return
            ...
            messagebox.showerror("Error", "Revisa que los datos de la Carga Objetivo sean números.")
    ...
    def dibujar_vectores(self, xq, yq, fx, fy):
        ...
        for c in self.cargas_fuente:
            ...
            self.ax.text(c['x'], c['y']+0.05, f" {signo}{c['q']:.1e}C", fontsize=9, ha='center')

        self.ax.scatter(xq, yq, color='green', s=150, marker='*', zorder=6, label='Carga Objetivo')
        
        if fx != 0 or fy != 0:
            longitud_maxima_ejes = max(abs(xq), abs(yq), 1)
            escala = max(abs(fx), abs(fy)) / (longitud_maxima_ejes * 0.3)
            flecha_x = fx / escala
            flecha_y = fy / escala
            self.ax.arrow(xq, yq, flecha_x, flecha_y, head_width=0.08, head_length=0.1, 
